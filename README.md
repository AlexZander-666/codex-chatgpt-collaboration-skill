...
- 只清理当前 Skill 自己写入的草稿，并只在用户消息角色中计算精确任务标记；
...
回归测试会检查引用文件、前置元数据、隐式调用策略、单次发送恢复状态、结构化完成判据和最终报告证据字段。发布前还应使用当前 Codex 环境中 `skill-creator/scripts/quick_validate.py` 验证 Skill 目录；真实浏览器调用仍需在已授权的 ChatGPT 会话中单独验证。
